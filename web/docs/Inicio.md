...
- Buscar dentro del buffer del terminal con `Ctrl+F`, **navegar entre prompts y salidas como bloques de comando** con `Alt+↑/↓` cuando el shell emite OSC 133, y filtrar perfiles desde la barra lateral.
...
- Guardar **snippets remotos** y **comandos locales** en Preferencias → Comandos. Los snippets se insertan en la terminal activa y se sincronizan cifrados; los comandos locales ejecutan shell, abren URLs o rutas en este equipo y no se sincronizan. Todo aparece en la **paleta de comandos** (`Ctrl+Shift+P`), junto a perfiles, scripts, notas y acciones contextuales, ordenado por tu uso reciente.
- Recibir **avisos con criterio**: fin de comandos largos (OSC 133) y de transferencias SFTP — silencio si lo estás mirando, toast si estás en otra pestaña, notificación del sistema si la app está detrás. Y lanzar una **comprobación de salud** (DNS/TCP, sin autenticar) sobre tus favoritos o un workspace.
...
- Gestionar en una sola sección **Preferencias → Credenciales** las credenciales locales del **keyring del sistema** (KWallet, GNOME Keyring, Keychain, Credential Store), las variables y una base **KeePass** (`.kdbx`), que puede **bloquearse sola** tras un rato sin usarla o al suspender el equipo.
- **Confirmar la huella del servidor en la primera conexión** (y avisar si cambia después): la clave de un host nuevo no se aprende en silencio salvo que tú lo elijas. Ver [Seguridad](?page=Seguridad).
- Sobrevivir a los sustos: si el fichero de conexiones se daña, Rustty **restaura la última copia válida** y te avisa (nunca borra el dañado); y tras suspender el equipo, comprueba qué sesiones siguen vivas y puede reconectar las caídas de forma escalonada.
