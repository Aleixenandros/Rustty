...
**Rustty** es un cliente de terminal y gestor de conexiones multiplataforma. Soporta **SSH**, **SFTP**, **FTP/FTPS** y **RDP** en la misma aplicación, con una implementación 100 % Rust del backend SSH/SFTP (sin dependencia de `libssh2`) y una interfaz moderna sobre **Tauri** + **xterm.js**.
...
- **Desconectar todo** de golpe desde el botón de emergencia del rail o un atajo configurable: cierra las sesiones SSH, SFTP/FTP, RDP y consolas locales, cierra los túneles y cancela las transferencias en curso, tras confirmar con el recuento afectado.
...
4. Para ver el panel de ficheros, haz clic en el icono **SFTP** dentro de la sesión SSH abierta, o crea un perfil FTP/FTPS si solo necesitas transferencia de archivos. Para túneles, usa el icono **⇄** de la pestaña o el acceso **⇄** del rail lateral.
